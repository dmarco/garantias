<template>
  <v-layout align-center justify-center pa-1>
    <v-flex xs12 sm10 md9 lg6 xl5>
      <v-card-title class="display-2 mt-4 mb-2 white--text">Bienvenido a Crédito y Caución SA</v-card-title>
      <v-card class="pa-3">
        <v-card-text class="mb-3">
          <p class="mb-4">Ingrese su dirección de e-mail y contraseña para loguearse en la plataforma y así poder guardar el estado de su formulario, gestionar su pago y descargar su garantía en cualquier momento. <strong>Jamás haremos uso comercial de su información privada
                      ni le enviaremos un email.</strong></p>
          <v-form v-model="valid" row lazy-validation ref="form">
            <v-layout row wrap>
              <v-flex xs12 md6 px-1>
                <v-text-field label="E-mail" :clearable="true" :persistent-hint="true" hint="Ingrese aquí su email" :validate-on-blur="true" v-model="email" :rules="emailRules" required ref="email"></v-text-field>
              </v-flex>
              <v-flex xs12 md6 px-1>
                <v-text-field label="Contraseña*" :persistent-hint="true" hint="Ingrese aquí su contraseña" :rules="passwordRules" v-model="password" min="8" max="100" :append-icon="p1 ? 'visibility' : 'visibility_off'" @click:append-icon-cb="() => (p1 = !p1)" :type="p1 ? 'password' : 'text'"></v-text-field>
              </v-flex>
        ...
        <v-card-actions>
          <v-layout row wrap class="mb-4 mt-4">
            <v-flex xs12 md8 px-2 mt-1 text-md-left text-xs-center>
        ...
        <v-card-actions>
          <v-layout row wrap class="mb-2 mt-2">
            <v-flex xs12 md12 px-2 mt-4 text-xs-center>
              <small>No tienes un usuario? </small>
              <v-btn outline color="secondary" to="/registro" nuxt>Crea uno aqui</v-btn>
            </v-flex>
          </v-layout>
        </v-card-actions>
      </v-card>
    ...
    methods: {
      checkCookie: function () {
        if (Cookie.get('auth')) {
          ...
          // this.$nuxt.$router.replace({ path: '/pasos' })
        }
      },
      login: function () {
